...
============================================================
  Fichero: zxou.c
  Creado: 18-10-2025
  Ultima Modificacion: dissabte, 8 de novembre de 2025, 18:00:58
  oSCAR jIMENEZ pUIG                                       
============================================================
*/
...
void x_pixel(short x,short y) {
	XSetForeground(display,graphic,X_BLACK);
	XFillRectangle(display,virtual,graphic,x,y,X_PIX,X_PIX);
}

void x_character(short x,short y) {
	const short FACT=X_PIX*8;
	XSetForeground(display,graphic,X_WHITE);
...
static void char_output(byte x,byte y) {
	...
	byte px=x*8;
	byte py=y*8;
	x_character(px*DPIX,py*DPIX);
	for(byte n=0;n<8;n++) {
		byte flagp=128;
		while(flagp) {
			if(flagp & *ppix) x_pixel(px*DPIX,py*DPIX);
			px++;
			flagp=flagp>>1;
		}
		py++;
...
void z_cls() {
	...
	do {
		printc(' ');
	} while (memory[CURSX]!=0 || memory[CURSY]!=0);
}
...
void z_mode(byte m) {
	if(m==INVERSE) *(memory+FLAG)|=FINV;
	else *(memory+FLAG)&=(~FINV);
}
...
void z_plot(byte x,byte y,byte s) {
	byte q=x/8;
	byte r=x%8;
	byte* dir=memory+OSCR+y*(SCRWP/8)+q;
	byte on=(s ^ (*(memory+FLAG) & FINV));
	if(on) *dir|=(128>>r);
	else *dir&=~(128>>r);
}
...
byte z_plotted(byte x,byte y) {
	...
	byte r=x%8;
	byte* dir=memory+OSCR+y*(SCRWP/8)+q;
	return (*dir & (128>>r))?1:0;
}

void z_printc(byte chr) {
	byte inv=(*(memory+FLAG) & FINV);
